...
export class ProfileAccountInformationComponent  implements OnInit {
  ...
  constructor(private service:KafaatMainService,private http: HttpClient,private adminService:MainDashoardService,
    private dateProvider:DateAdapter<Date>,private jak:NgxMatDateAdapter<Date>,private cdr: ChangeDetectorRef){
  ...
  async changeGender() {
    const genderValue = this.profile.gender;
    console.log("Gender value:", genderValue);

    const userImageIsDefault =
     ...
     if (genderValue === 'm' && !userImageIsDefault) {
      console.log("Setting male default image...");
      await this.setGenderImageAndUpload('/assets/images/male.png');
    } else if (genderValue === 'f' && !userImageIsDefault) {
      console.log("Setting female default image...");
      await this.setGenderImageAndUpload('/assets/images/female.png');
    } else {
      console.log("Gender change ignored, custom image in use.");
    }
  }
  async setGenderImageAndUpload(imagePath: string) {
    try {
      console.log("Fetching and converting image at path:", imagePath);
      const imageFile = await this.urlToFile(imagePath);
      this.userProfileImage = URL.createObjectURL(imageFile);
      const mockEvent = {
        target: {
          files: [imageFile]
        }
      };
      this.onUserImageSelected(mockEvent);
    } catch (error) {
      console.error("Error in setGenderImageAndUpload:", error);
    }
  }
  ...
  async urlToFile(url: string): Promise<File> {
    try {
      ...
      console.log("Image successfully fetched and converted to blob.");
      return new File([blob], 'genderImage.png', { type: blob.type });
    } catch (error) {
...
valueEmitted(event: any) {
  ...
  changeCountry() {
    const selectedCountryId = this.profile.countryId;
    ...
    const countryCodes: { [key: number]: string } = {
      2: '966',
        10003: '965',
        10002: '973'
      // Add other countries and their codes if needed
    };

    const countryCode = countryCodes[selectedCountryId] || '';

    // Set default value with country code
    if (countryCode) {
      this.profile.phoneNumber = countryCode; // Pre-fill with country code
    } else {
      this.profile.phoneNumber = ''; // Clear phone number if country code is not defined
    }

    // Load cities based on the selected country
    this.loadAllCitiesRelated(selectedCountryId);
  }
  ...
  //   } else if (phoneNumber && !phoneNumber.startsWith(countryCode)) {
...
constructFormData(): FormData {
  ...
  formData.append('id',model.email);
